 ...
 **/
app.factory('TransactionService', ['$resource', '$q', function($resource, $q) {
	...
	var intializeTransaction = function() {
		...
		var Buyer = $resource('/app/data/cost.json');

		return Buyer.get({
		}, function (response) {
	  		
	  		healthySnackCost = response.healthySnack;
		  	treatCost = response.treat;
		  	mediumLemonadeCost = response.mediumLemonade;
		  	largeLemonadeCost = response.largeLemonade;
		  	transactionInitialized = true;
			//deferred.resolve(response);

		}, function() {
			//deferred.reject('couldn\'t initialize transaction');
		});
